...
Starting with Cloud Volumes ONTAP 9.8 in AWS, new deployments of Cloud Volumes ONTAP no longer support the c4, m4, and r4 EC2 instance types. You can upgrade existing deployments to Cloud Volumes ONTAP software versions 9.8 and later with these instance types but you should make sure to change the instance type and enable _Enhanced networking_ to avoid upgrade issues. 

==== Change the instance type
c4, m4, and r4 EC2 instance types allow for more disks per node than the c5, m5, and r5 EC2 instance types. If you have a node-based license and if the disk count per node for the c4, m4, or r4 EC2 instance you're running is below the max disk allowance per node for c5, m5, and r5 instances, you can change the EC2 instance type to c5, m5, or r5.
