...
. Optionally, you can click *All versions* to enter another compatible version that you want to upgrade to (say, the next patch of the installed version). 
. Click *Save*, and then *Apply*. 
+
For the appropriate upgrade path of your current Cloud Volumes ONTAP version, refer to link:task-updating-ontap-cloud.html#supported-upgrade-paths[Supported upgrade paths].
+
