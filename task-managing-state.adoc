...
*Example*
+
The following image shows a schedule that instructs BlueXP to shut down the system every Saturday at 12:00 a.m. for 48 hours. BlueXP restarts the system every Monday at 12:00 a.m.
+
...
BlueXP saves the schedule. The clock icon changes to indicate that a schedule is set: image:screenshot_shutdown_icon_scheduled.gif[Screen shot: Shows the automatic shutdown icon after a schedule is defined.]
