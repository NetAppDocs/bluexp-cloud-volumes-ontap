...
keywords: Google Cloud Platform, image, VHD, image signature file, comvert, raw format,
summary: Convert an exported image into disk.raw format through the Google Cloud Platform command line. 
---

= Convert image to Raw format on GCP
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
The image being used to deploy new instances or upgrades or being used in existing images will be shared with the clients through https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab[the NetApp Support Site (NSS)]. The signed digest, and the certificates will be available to download through the NSS portal. Make sure you are downloading the digest and certificates for the right release corresponding to the image shared by NetApp Support. For instance, 9.13.0 images will have a 9.13.0 signed digest and certificates available on NSS.

== Why is this step needed?
The images from GCP cannot be downloaded directly. In order to verify the image against the signed digest and the certificates, you need to have a mechanism to compare the two files and download the image. To do so, you must export/convert the image into a disk.raw format and save the results in a storage bucket on GCP. The disk.raw file is tarred and gzipped in the process.
...
== Use GCP command line to export GCP image
...
NOTE: See https://cloud.google.com/compute/docs/images/export-image#console[Google Cloud Platform doc on Exporting an image] for more information on how to export an image through GCP.  