...
. On the Details & Credentials page, subscribe to the offering in your cloud provider’s marketplace.
...
ifdef::aws[]
* In AWS, annual contracts are available from a public offer in the AWS Marketplace. Cloud Manager prompts you to subscribe from the Details & Credentials page when deploying a Cloud Volumes ONTAP working environment.
endif::aws[]
