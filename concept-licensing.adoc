...
Beginning with 11-November-2024, the limited availability of node-based licenses has been terminated. Support for node-based licensing ends on 31-December-2024.
...
a| You won't be entitled to deploy new Cloud Volumes ONTAP systems using that license. The existing system might continue to work, but you won't receive any support or updates for your system post the EOS date.
...
.Exceptions
NetApp recognizes that certain situations require special consideration, and EOA and end of support (EOS) of node-based licensing will not apply to the following exceptions:
...
[NOTE]
In these scenarios, new node-based licenses and license renewals are valid for a maximum of one year starting from the date of approval.


== License conversion

BlueXP enables a seamless conversion of node-based licenses to capacity based by the license conversion tool. For information about EOA of node-based licensing, refer to link:/concept-licensing.html#end-of-availability-of-node-based-licenses[End of availability of node-based licenses].
