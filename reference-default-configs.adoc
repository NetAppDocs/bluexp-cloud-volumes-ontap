...
+
NOTE: Snapshots are created automatically upon reboot. In some zones, the available EBS disk type can only be gp2.

* For HA pairs, two EBS volumes for the mediator instance, which are approximately 8 GiB and 4 GiB, and an additional 140 GiB gp3 disk in each node to contain a copy of the root data of the other node.
