...
| Used to retrieve the client secret key from the Azure Key Vault to communicate with S3 bucket for metadata handling. Cloud Volumes ONTAP uses this Azure endpoint for Google Cloud.
...
| Used by Cloud Volumes ONTAP to communicate with the Google Cloud providers. 

| Calls to Google Cloud providers.

|Inability of Cloud Volumes ONTAP to manage specific functionalities of Google Cloud providers.  
