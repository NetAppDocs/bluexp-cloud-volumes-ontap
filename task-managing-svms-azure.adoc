...
Multiple storage VMs are supported with Cloud Volumes ONTAP in Azure starting with the 9.9.0 release. Go to the https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/index.html[Cloud Volumes ONTAP Release Notes^] to verify the supported number of storage VMs for your version of Cloud Volumes ONTAP.
...
.iSCSI
--
* One IP address for data LIF access
...
How you create a storage VM and LIFs depends on the storage protocol that you're using.
...
One iSCSI data LIF is required on each node. A storage VM (SVM) management LIF is optional. This management LIF provides a connection to management tools like SnapCenter.
...
One NAS data LIF is required on each node. A storage VM (SVM) management LIF is optional. This management LIF provides a connection to management tools like SnapCenter.
...
* One NAS data LIF is required on each node.
* One iSCSI LIF is required on each node.
+
The iSCSI LIF is required for DNS and SMB communication.
* A storage VM (SVM) management LIF is optional.
+
This management LIF provides a connection to management tools like SnapCenter.
...
.. Use the following command to create an iSCSI LIF on node 1, which is required to provide DNS and SMB communication:
+
[source,cli]
network interface create -vserver <svm-name> -home-port e0a -address <iscsi-ip-address> -lif <lif-name> -home-node <name-of-node1> -data-protocol iscsi
