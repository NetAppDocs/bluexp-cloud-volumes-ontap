...
.Steps
. In Azure, go to Azure services and select *Managed Identites*. 
. Click *Create*.
...
+
.Footnote
`userAssignedIdentityId` is an optional parameter. Include the parameter if you created this resource to be used for storage account encryption. 
+
