...
[NOTE]
After configuring, ensure that you have https://learn.microsoft.com/en-us/azure/virtual-machines/disk-encryption#customer-managed-keys[enabled key rotation in the Azure portal^], so that Cloud Volumes ONTAP can utilize the new keys. 

On creating and enabling a new version of your key, Cloud Volumes ONTAP automatically detects and uses the latest version of the key for encryption, ensuring your data remains secure without the need for manual updates. 
