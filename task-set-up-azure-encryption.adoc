...
Azure HA multi availability zone::

* All Azure storage accounts for Cloud Volumes ONTAP are encrypted using a customer-managed key.*** 

* For root, boot, NVRAM, core and data disks, Cloud Manager uses a disk encryption set, which enables management of encryption keys with managed disks.
...
NOTE: The first three permissions are not required for HA [pairs below ONTAP 9.10.1P3].
