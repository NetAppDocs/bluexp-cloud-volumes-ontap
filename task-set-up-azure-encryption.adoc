...
[NOTE]
After configuring the keys, ensure that you have selected https://learn.microsoft.com/en-us/azure/key-vault/keys/how-to-configure-key-rotation#key-rotation-policy[_Enable auto rotation_^], so that Cloud Volumes ONTAP can use new keys when the previous keys expire. If you don't enable this option on the Azure portal, Cloud Volumes ONTAP can't automatically detect the new keys, which might cause issues with storage provisioning.
