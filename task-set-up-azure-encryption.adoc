...
You should configure your encryption keys on the Azure portal. For information about configuring your keys and setting up key rotation, refer to the following Microsoft Azure documentation topics:
...
[NOTE]
After configuring the keys, ensure that you have https://learn.microsoft.com/en-us/azure/virtual-machines/disk-encryption#customer-managed-keys[enabled key rotation on the Azure portal^], so that Cloud Volumes ONTAP can utilize the new keys. 

On creating and enabling a new version of your keys, Cloud Volumes ONTAP automatically detects and uses the latest version for encryption, ensuring your data remains secure without the need for manual updates.
