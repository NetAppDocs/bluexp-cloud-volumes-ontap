...
##What you'll need::##
Before enabling high availability mode in a zonal deployment in Azure, the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature should be activated for your subscription. This feature facilitates the deployment of VM instances by Azure Virtual Machine Scale Sets in Flexible orchestration mode in single availability zones. Perform these steps as a user with "Owner" privileges from Azure CLI:
