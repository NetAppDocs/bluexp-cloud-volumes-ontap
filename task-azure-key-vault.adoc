...
. In the Azure Overview window, select the application you have registered. Copy the **application client ID** (_Azure_client_ID_) and the **directory tenant ID** (_Azure_tenant_ID_) to a secure location. They will be required later in the registration process.
...
. Select **Add** to save the client secret.	Immediately copy the **Value** (_Azure_client_secret_) of the secret and store it somewhere secure for future configuration. The secret value will not be displayed after you navigate away from the page.
...
. Record the Key Vault URI: In the key vault you created, navigate to the Overview menu and copy the **Vault URI** (_Azure_keyvault_uri_) from the right-hand column. You will need this for a later step.
...
. Locate the **Key Identifier** field. Copy the URI (_Azure_key_ID_) up to but not including the hexadecimal string.
...
`security key-manager external azure enable -vserver _SVM_name_ -client-id _Azure_client_ID_ -tenant-id _Azure_tenant_ID_ -name _Azure_keyvault_uri_ -key-id _Azure_key_ID_`
.. Enter the _Azure_client_secret_ when prompted by the CLI as it processes the Enable command.
...
----
+
If the `service_reachability` status is not `OK`, the SVM cannot reach the Azure Key Vault service with all the required connectivity and permissions. Check your Azure network policies and routing are not blocking your private vNet from reaching the Azure KeyVault Public endpoint. If it is, consider using an Azure Private endpoint to access the Key vault from within the vNet, you may also need to add a static hosts entry on your SVM to resolve the private IP address for your endpoint.
