...
| Auth failures will occur and result in a denial of service for the following:
    * OCCM
    * ontap-service
    * protocol-proxy
...
| Used to retrieve the client secret from the Azure vault to communicate with S3 bucket for metadata handling purposes.

| Secret token retrieved from Azure Vault. 

| Service for OCCM will not be available.  
...
| Used to retrieve the Cloud Volumes ONTAP resources from the Tenancy to authorize the resource and user.

This endpoint does not apply if you are using BlueXP in private mode.

| Retrieval of the resource data. 

| Resources for the user and Cloud Volumes ONTAP are not authorized.  
...
| AutoSupport information delivered to NetApp

| AutoSupport telemetry is not delivered to NetApp. 
...
| 
The exact endpoint depends on the AWS region that you are using. Refer to https://docs.aws.amazon.com/general/latest/gr/rande.html[AWS documentation for details^]. 

| Used by Cloud Volumes ONTAP to talk to the AWS cloud provider. 

| AWS provider call

| Management of AWS provider functionalities in OCCM will be unavailable. 
