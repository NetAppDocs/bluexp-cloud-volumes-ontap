...
| Used to retrieve client secret key from the Azure Key Vault to communicate with S3 buckets for metadata handling. Cloud Volumes ONTAP uses this Azure endpoint for AWS.
...
a| 
Endpoints for AWS services (amazonaws.com):
...
| Used by Cloud Volumes ONTAP to communicate with the AWS cloud provider. 

| Calls to AWS cloud providers.

|Inability of Cloud Volumes ONTAP to manage specific functionalities of AWS cloud providers.
