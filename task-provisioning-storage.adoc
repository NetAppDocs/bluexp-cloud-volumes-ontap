---
sidebar: sidebar
permalink: task-provisioning-storage.html
...
. Add values for all of the parameters that are not hard-coded from the template. See <<Create FlexVol volumes,creating volumes>> for details about all the parameters you need to complete to deploy a Cloud Volumes ONTAP volume.
