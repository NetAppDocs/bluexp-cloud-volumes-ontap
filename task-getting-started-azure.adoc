...
##For zonal deployments in locally redundant storage (LRS) single availability zones, you should subscribe to the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature. For instructions, refer to link:task-azure-high-availability-mode.html[Enable high availability mode in Azure].##
