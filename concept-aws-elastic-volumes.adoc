...
These permissions are included in https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html[the policies provided by NetApp^]
...
Increasing the size of an aggregate can take up to 6 hours. During that time, Cloud Manager can't request any additional capacity for that aggregate.
