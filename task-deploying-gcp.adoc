...
link:concept-write-speed.html[Learn more about write speed].
+
NOTE: _Flash cache_, high write mode, and a higher maximum transmission unit (MTU) of 8,896 are available through the *High* write speed option with the n2-standard-16, n2-standard-32, n2-standard-48, and n2-standard-64 instance types. In addition, the higher MTU of 8,896 requires the selection of VCP-1, VPC-2, or VCP-3 for the deployment. _Flash cache_, high write mode, and an MTU of 8,896 are feature-dependent and cannot be disabled individually within a configured instance. 
.. Activate write once, read many (WORM) storage, if desired.
+
WORM can't be enabled if data tiering was enabled for Cloud Volumes ONTAP versions 9.7 and below. Reverting or downgrading to Cloud Volumes ONTAP 9.8 is blocked after enabling WORM and tiering. 
+
