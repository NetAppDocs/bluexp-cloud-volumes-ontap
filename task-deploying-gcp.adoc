...
Create a working environment in BlueXP to launch Cloud Volumes ONTAP in GCP.
...
| Working Environment Name | BlueXP uses the working environment name to name both the Cloud Volumes ONTAP system and the GCP VM instance. It also uses the name as the prefix for the predefined security group, if you select that option.
...
| Add Labels |	Labels are metadata for your GCP resources. BlueXP adds the labels to the Cloud Volumes ONTAP system and GCP resources associated with the system.
...
.. Click *More information* to review details about support and the GCP resources that BlueXP will purchase.
