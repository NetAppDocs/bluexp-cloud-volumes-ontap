...
+
For more information, refer to  link:task-planning-your-config-gcp.html#choose-a-volume-usage-profile[Choose a volume usage profile^],  link:concept-data-tiering.html[Data tiering overview^], For information about inline storage efficiency features, and https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#[KB: What Inline Storage Efficiency features are supported with CVO?]
