...
If you don't have a link:concept-connectors.html[Connector] yet, an Account Admin needs to create one. link:task-creating-connectors-gcp.html[Learn how to create a Connector in GCP].
...
[role="quick-margin-para"]
Cloud Volumes ONTAP requires a Google Cloud service account for two purposes. The first is when you enable link:concept-data-tiering.html[data tiering] to tier cold data to low-cost object storage in Google Cloud. The second is when you enable the link:concept-backup-to-cloud.html[Cloud Backup Service] to back up volumes to low-cost object storage.
...
* link:task-creating-connectors-gcp.html[Creating a Connector from Cloud Manager]
