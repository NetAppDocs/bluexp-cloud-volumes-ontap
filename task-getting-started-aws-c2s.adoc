...
//}
[NOTE]
For the policies that you just created to support the buckets that you create later, ensure that the bucket names are prefixed with `fabric-pool-`. This applies to the policies for both AWS Secret Cloud and Top Secret Cloud.
+
...
[NOTE]
For Top Secret Cloud, you should not upload any additional certificate to `Amazon Root 4` certificate that you have already installed in the previous step.
