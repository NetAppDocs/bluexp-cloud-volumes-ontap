...
. Installing the Connector and setting up BlueXP.
+
...
. Launching Cloud Volumes ONTAP from BlueXP.
...
Set up IAM policies and roles that provide BlueXP and Cloud Volumes ONTAP with the permissions that they need to perform actions in the AWS Commercial Cloud Services environment.
...
. Add the IAM role for the BlueXP instance to the list of key users for a CMK.
+
This gives BlueXP permissions to use the CMK with Cloud Volumes ONTAP.

== Install and set up BlueXP
...
.. Complete the steps in the Setup wizard to set up BlueXP.
+
* *System Details*: Enter a name for this instance of BlueXP and provide your company name.
* *Create User*: Create the Admin user that you'll use to administer BlueXP.
...
You can launch Cloud Volumes ONTAP instances in the AWS Commercial Cloud Services environment by creating new working environments in BlueXP.
...
BlueXP creates security groups that include the inbound and outbound rules that BlueXP and Cloud Volumes ONTAP need to operate successfully in the cloud. You might want to refer to the ports for testing purposes or if you prefer to use your own security groups.
