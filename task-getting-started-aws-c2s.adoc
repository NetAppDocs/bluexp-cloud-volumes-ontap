...
+
[CAUTION]
While making selections through the wizard, do not select *Data Sense & Compliance* and *Backup to Cloud* under *Services*. Under *Preconfigured Packages* select *Change Configuration* only, and ensure that you haven't selected any other option. These preconfigured packages aren't supported for AWS Secret Cloud and Top Secret Cloud regions, and if selected, your deployment will fail.
