...
= Get started with Cloud Volumes ONTAP in the AWS Top Secret Cloud environment
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Similar to a standard AWS region, you can use BlueXP in link:https://aws.amazon.com/federal/top-secret-cloud/[AWS Top Secret Cloud^] to deploy Cloud Volumes ONTAP, which provides enterprise-class features for your cloud storage. AWS Top Secret Cloud is a closed region specific to the U.S. Intelligence Community; the instructions on this page only apply to AWS Top Secret Cloud region users.

== Supported versions in AWS Top Secret Cloud	

* Cloud Volumes ONTAP 9.12.1 P1 is supported
 
* Version 3.9.32 of the Connector is supported
+ 
The Connector is software that's required to deploy and manage Cloud Volumes ONTAP in AWS. You'll log in to BlueXP from the software that gets installed on the Connector instance. The SaaS website for BlueXP isn't supported in AWS Top Secret Cloud.

== Private mode
When BlueXP deploys Cloud Volumes ONTAP in AWS Secret Cloud and Top Secret Cloud, deployment is in private mode. In private mode, a BlueXP Connector is installed on premises or in the cloud (in a secure region, sovereign cloud region, or commercial region) and has no connectivity to the BlueXP SaaS layer. Users access BlueXP locally from the web-based console that's available from the Connector, not from the SaaS layer.
...
Getting started with Cloud Volumes ONTAP in AWS Top Secret Cloud includes a few steps.

. <<Prepare your AWS environmentPreparing your AWS environment>>
+
...
. <<Install the Connector and set up BlueXP,Installing the Connector and setting up BlueXP>>
+
Before you can start using BlueXP to deploy Cloud Volumes ONTAP, you'll need to create a _Connector_. The Connector enables BlueXP to manage resources and processes within your public cloud environment (this includes Cloud Volumes ONTAP).
+
You'll log in to BlueXP from the software that gets installed on the Connector instance.

. <<Launch Cloud Volumes ONTAP from BlueXP,Launching Cloud Volumes ONTAP from BlueXP>>
...
A Marketplace subscription is required to deploy Cloud Volumes ONTAP from BlueXP.
...
+
You must use BlueXP to launch Cloud Volumes ONTAP instances. You must not launch Cloud Volumes ONTAP instances from the EC2 console.

=== Set up permissions

Set up IAM policies and roles that provide the Connector and Cloud Volumes ONTAP with the permissions that they need to perform actions in the AWS Top Secret Cloud.
...
+
You can select the IAM roles for Cloud Volumes ONTAP and the HA mediator when you create a Cloud Volumes ONTAP working environment from BlueXP.
...
== Install the Connector and set up BlueXP

Before you can launch Cloud Volumes ONTAP systems in AWS, you must first launch the Connector instance from the AWS Marketplace and then log in and set up BlueXP.
...
== Install a private mode certificate
To complete Connector installation and BlueXP setup, you need to manually install a private mode certificate using Docker.

.Steps
. List existing installed certificates. 
.. To collect the occm container docker id (identified name “ds-occm-1”), run `docker ps`.
.. To get inside occm container, run `docker exec -it <docker-id> /bin/sh`.
.. To collect the password from “TRUST_STORE_PASSWORD” environment variable, run `env`.
.. To list all installed certificates in truststore, run `keytool -list -v -keystore occm.truststore` and use the password collected in the previous step. 

. Add a certificate. 
.. To collect occm container docker id (identified name “ds-occm-1”), run `docker ps`.  
.. To get inside occm container, run `docker exec -it <docker-id> /bin/sh`.  
Save the new certificate file inside.
.. To collect the password from “TRUST_STORE_PASSWORD” environment variable, run  `env`. 
.. To add the certificate to the truststore, run `keytool -import -alias <alias-name> -file <certificate-file-name> -keystore occm.truststore` and use the password collected in the previous step. 
.. To check that the certificate installed, run `keytool -list -v -keystore occm.truststore -alias <alias-name>`. 
.. To exit occm container, run `exit`.
.. To reset occm container, run `docker restart <docker-id>`.  

== Launch Cloud Volumes ONTAP from BlueXP

You can launch Cloud Volumes ONTAP instances in the AWS Top Secret Cloud by creating new working environments in BlueXP.

.Before you begin

* If you purchased a license from NetApp, you need to add it to the BlueXP digital wallet. link:https://docs.netapp.com/us-en/bluexp-digital-wallet/task-manage-node-based-licenses.html#add-unassigned-licenses[Learn how to add a license^].
...
The predefined internal security group for the Cloud Volumes ONTAP HA mediator includes the following rules. BlueXP always creates this security group. You don't have the option to use your own.
