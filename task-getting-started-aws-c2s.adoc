...
+
See https://docs.netapp.com/us-en/bluexp-s3-storage/task-add-s3-bucket.html[Add S3 buckets].
...
[NOTE]
All Amazon Resource Names (ARNs) should be suffixed with `-iso-b`, such as `arn:aws-iso-b`. For example, if a resource requires an ARN with a region, use the naming convention as `us-iso-b` for the `-server` flag.
+
----
storage aggregate object-store config create -object-store-name <S3Bucket> -provider-type AWS_S3 -auth-type EC2-IAM -server <s3.us-isob-east-1.server_name> -container-name <fabric-pool-testbucket> -is-ssl-enabled true -port 443
----
+
