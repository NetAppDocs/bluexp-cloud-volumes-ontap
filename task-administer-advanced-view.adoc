...
* SMB Continuous Availability (CA) is not supported in Cloud Volumes ONTAP. You can't create continuously available SMB shares for nondisruptive operations. For information about continuously available SMB shares, refer to https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/What_are_SMB_Continuous_Availability_CA_Shares[What are SMB Continuous Availability (CA) Shares?]   
