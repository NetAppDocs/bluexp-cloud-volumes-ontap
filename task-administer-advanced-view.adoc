...
. Click the action menu image:icon-action.png[ellipses icon] for a Connector and select *Edit Connector*.
. Under *Force user credentials for cluster login via System Manager*, select the *Enable/Disable* check box. By default, authentication is disabled.
+
[NOTE]
