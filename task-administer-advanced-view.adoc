...
* BlueXP tiering
+
The BlueXP tiering service is not supported with Cloud Volumes ONTAP. You should set up tiering of data to object storage directly from BlueXP's Standard View when creating volumes.
...
As an administrator, you can activate user authentication for accessing System Manager from BlueXP. You can determine the right level of access permissions based on the ONTAP user roles, and enable or disable authentication as needed. If you enable authentication, then users need to enter their System Manager credentials every time they access it from BlueXP, because BlueXP doesn't store the credentials internally. If you disable authentication, then users can access System Manager by using BlueXP admin credentials.
...
. ##If prompted, enter your System Manager credentials and click *Login*.##
