...
You now have the ability to activate write once, read many (WORM) storage on an existing Cloud Volumes ONTAP system through BlueXP. This feature provides you with the flexibility of enabling WORM on a working environment, even if WORM was not enabled on it during creation. Once enabled, you cannot disable WORM.
