...
. Go to `/opt/application/netapp/cloudmanager/docker_occm/data/` where the proxy configuration file `squid.conf` resides.
+
...
. If the CIDR block of the Cloud Volumes ONTAP system isn't in that range, either update the value or add a new entry as follows:
+
`acl cvonet src <cidr>`
+
...
. Verify again that Cloud Volumes ONTAP can send AutoSupport messages:
+
`autosupport check show-details`
