...
* The tool supports all node-based licenses as the source and all capacity-based licenses as the destination. ##For example, if you have a PAYGO Standard node-based license, you can convert it to any capacity-based license purchased through the marketplace. NetApp has restricted purchase, extension, and renewal of BYOL licensing. For information, refer to https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/whats-new.html#end-of-availability-of-byol-licensing-for-cloud-volumes-ontap[End of availability of BYOL licensing for Cloud Volumes ONTAP^].##
