...
* The license conversion tool supports all CVO deployment types, such as single node, high availability (HA) in single availability zone (AZ), HA in multiple AZ, BYOL, and PAYGO.
...
[NOTE]
If any of these requirements is not fulfilled, the license conversion does not happen. In specific cases, the license might be converted, but cannot be used. Click the information icon to view the appropriate messaging and identify the issues to take corrective actions.
...
+
[TIP]
The icon is disabled if your Cloud Volumes ONTAP system is already charged by capacity, or if any of the requirements is not met. 
+
