...
ifdef::aws[]
For AWS, this typically means using AWS Key Management Service (KMS).
endif::aws[]
ifdef::azure[]
For Azure, you need to set up Azure Key Vault for NetApp Volume Encryption (NVE).
endif::azure[]
ifdef::gcp[]
For Google Cloud, it is Google Cloud Key Management Service.
endif::gcp[]
...
. *ONTAP version*: Ensure that the ONTAP version of your Cloud Volumes ONTAP system supports FlexCache with encrypted volumes. See the latest https://docs.netapp.com/us-en/ontap/release-notes/index.html[ONTAP release notes] or compatibility matrix for more information.
...
.Steps
Create a FlexCache volume on your Cloud Volumes ONTAP system with an encrypted source volume. For detailed steps and additional considerations, see:
