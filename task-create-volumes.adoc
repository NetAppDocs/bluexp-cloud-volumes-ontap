...
. If there are no other Actions that you need to define (for example, configuring BlueXP backup and recovery (Cloud Backup)), click *Save Template*.
+
...
Additionally, if any secondary action is implemented in the template, for example, enabling BlueXP backup and recovery (Cloud Backup) on the volume, that action is also performed.
