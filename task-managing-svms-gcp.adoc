...
Note that Cloud Manager allocates the required IP addresses in Google Cloud and creates the storage VM with a data LIF for SMB/NFS access and an iSCSI LIF for outbound SMB communication.
...
These permissions are included in https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-gcp.html[the policies provided by NetApp].
