...
Cloud Volumes ONTAP supports only HA deployments in AWS Local Zones. The following are example configurations: 
...
Cloud Volumes ONTAP supports only these configurations. Selecting any other disk type or EC2 instance in AWS Local Zone configuration for Cloud Volumes ONTAP might result in deployment failure. Refer to AWS documentation for the latest and complete details of the link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2[EC2 instance types in Local Zones^].
