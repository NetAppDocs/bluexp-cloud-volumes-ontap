...
Access to the instance:: When you create a Cloud Volumes ONTAP HA pair from Cloud Manager, you're prompted to provide a key pair for the mediator instance. You can use that key pair for SSH access using the `admin` user.
...
For details, see link:reference-networking-aws.html#aws-networking-requirements-for-cloud-volumes-ontap-ha-in-multiple-azs[AWS networking requirements for Cloud Volumes ONTAP HA in multiple AZs].
