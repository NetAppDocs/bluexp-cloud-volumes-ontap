...
AWS Local Zones are a kind of infrastructure deployment where storage, compute, database, and other select AWS services are located close to large cities and industry areas. With AWS Local Zones, you can bring AWS services closer to you which improves latency for your workloads and maintain databases locally. 
...
** One cluster node and mediator are in Local Zone 1 and the second cluster node is in Local Zone 2.
...
The following EC2 instance type families with sizes xlarge to 4xlarge are supported: 
