...
[.lead]
You can verify an exported VHD file signature on macOS by following these steps.
...
----
+
. Remove 1MB (1,048,576 bytes) at the beginning and 512 bytes at the end of the VHD file.
When using `tail`, the `-c +K` option generates bytes from the Kth byte of the file. Therefore, it passes 1048577 to `tail -c`. Note that on macOS, the tail command may take a little while to complete.
+
[source,cli]
----
