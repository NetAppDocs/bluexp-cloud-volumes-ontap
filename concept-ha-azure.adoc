...
##Azure Virtual Machine Scale Sets in Flexible orchestration mode helps to create and manage groups of heterogeneous, auto-scaling virtual machines (VMs) in a zonal deployment model for a single availability zone. The number of VM instances can automatically increase or decrease, or balance load based on demands or scheduled conditions. In this orchestration mode, distinct fault domains are defined for each zone, ensuring high availability at scale. This is applicable to Cloud Volumes ONTAP 9.15.1 and later. To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^].##
...
Availability Zones::
##HA multiple availability zone configuration utilizes a deployment model where the concept of availability zone is used. In this model, two Cloud Volumes ONTAP nodes are deployed into different availability zones, ensuring that the nodes are in different fault domains to provide redundancy and availability. To learn mode about availability sets, refer to https://learn.microsoft.com/en-us/azure/virtual-machines/availability-set-overview[Availability sets overview^].##
