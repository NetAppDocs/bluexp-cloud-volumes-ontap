...
##Azure Virtual Machine Scale Sets in Flexible orchestration mode help to create and manage groups of heterogeneous, auto-scaling virtual machines (VMs) in a zonal deployment mode for a single availability zone. The VMs are deployed into distinct fault domains to ensure high availability. This is applicable to Cloud Volumes ONTAP 9.15.1 and later. To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^].##
