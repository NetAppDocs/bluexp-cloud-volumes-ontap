...
##Cloud Volumes ONTAP 9.15.1 and later support creation and management of heterogeneous, auto-scaling virtual machines (VMs) in a zonal deployment mode for single availability zones. The Flexible orchestration mode of Azure Virtual Machine Scale Sets helps to deploy VMs into distinct fault domains within the same availability zone to ensure high availability. To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^].##
