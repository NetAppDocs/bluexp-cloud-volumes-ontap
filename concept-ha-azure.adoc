...
##Cloud Volumes ONTAP 9.15.1 and later supports the creation and management of heterogeneous, auto-scaling virtual machines (VMs) in a single availability zone by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. In this mode, you can deploy HA nodes in distinct fault domains in the same availability zone, to ensure high availability. To learn more, refer to the Microsoft https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Microsoft Azure documentation: Virtual Machine Scale Sets documentation^].##
...
VMs in single availability zones::
##Cloud Volumes ONTAP 9.15.1 and later supports the creation and management of heterogeneous, auto-scaling virtual machines (VMs) in a single availability zone by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. In this mode, you can deploy HA nodes in distinct fault domains in the same availability zone, to ensure high availability. To learn more, refer to https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Microsoft Azure documentation: Virtual Machine Scale Sets^].##
