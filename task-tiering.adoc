...
[role="quick-margin-para"]
Data tiering should be enabled on an aggregate to enable it on a volume. You should be aware of the requirements for new volumes and for existing volumes. <<Ensuring that tiering is enabled on aggregates,Learn more>>.
...
=== Requirements to tier data for a different Azure tenant
This scenario applies if the Cloud Volumes ONTAP working environment is created by a tenant and the Connector is created by another. If you want to enable tiering for a different tenant, then ensure that:
...
* link:task-enabling-private-link.html#disable-azure-private-links-and-use-service-endpoints-instead[Disable Azure Private Links and use the Subnet Service Endpoint instead].
