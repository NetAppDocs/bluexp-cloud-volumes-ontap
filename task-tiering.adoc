...
When you decommission a Cloud Volumes ONTAP environment, the S3 bucket that was created for the environment is not deleted automatically. You should delete the S3 bucket by using ONTAP CLI or APIs. You can delete the S3 bucket only if:
...
If the Cloud Volume ONTAP working environment is decommissioned, but has an S3 bucket with objects associated with it, the bucket will not be deleted to prevent any unintended data loss. It will be left in an orphaned state until all the objects are removed, following which, the bucket can be deleted.
