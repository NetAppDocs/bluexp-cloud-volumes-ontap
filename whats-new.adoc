...
Flash Cache and high write speed can now be enabled separately in Google Cloud. High write speed is available on all supported instance types. Flash Cache is supported on the following instance types: 
...
You can use these features separately or together on both single node systems and high-availability deployments. 
