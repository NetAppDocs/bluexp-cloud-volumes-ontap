...
Two new licensing packages are available in the Azure Marketplace as a PAYGO offering:

Optimized package::
The same capabilities as the Essentials package with charging based on storage and usage (I/O).

Edge Cache package::
The same capabilities as the Professional package with a Global File Cache license.

link:concept-licensing.html#capacity-based-licensing-packages[Learn more about these packages].
