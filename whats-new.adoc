...
The n1 series machines are no longer supported in new deployments of Cloud Volumes ONTAP in Google Cloud, and are retained only in older systems. New deployments of Cloud Volumes ONTAP are supported in Google Cloud only from the 9.8 release. We recommend that you switch to the n2 series machine types that are compatible with Cloud Volumes ONTAP 9.8 and later.
