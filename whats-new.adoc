...
The minimum ONTAP version required for configuring Flash Cache on Azure is 9.13.1 GA. Only ONTAP 9.13.1 GA and later versions can be used for deploying Flash Cache on Cloud Volumes ONTAP systems for Azure. For supported configuration, refer to https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-configs-azure.html#single-node-systems[Supported configurations in Azure].
