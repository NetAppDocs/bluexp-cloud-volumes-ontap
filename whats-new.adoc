...
=== WORM + BlueXP backup and recovery (Cloud Backup) now available during Cloud Volumes ONTAP creation 

The ability to activate both write once, read many (WORM) and BlueXP backup and recovery (Cloud Backup) features is now available during the Cloud Volumes ONTAP creation process. 
...
=== BlueXP digital wallet enhancements
...
You can now change the charging method for a Cloud Volumes ONTAP system that uses capacity-based licensing. For example, if you deployed a Cloud Volumes ONTAP system with the Essentials package, you can change it to the Professional package if your business needs changed. This feature is available from the BlueXP digital wallet.
...
The BlueXP digital wallet now shows you the total consumed capacity in your account and the consumed capacity by licensing package. This can help you understand how you're being charged and whether you need to purchase additional capacity.

image:https://raw.githubusercontent.com/NetAppDocs/cloud-manager-cloud-volumes-ontap/main/media/screenshot-digital-wallet-summary.png["A screenshot that shows the BlueXP digital wallet page for capacity-based licenses. The page provides an overview of the consumed capacity in your account and then breaks down the consumed capacity by licensing package."]
...
If you have an unassigned node-based license for Cloud Volumes ONTAP that you haven't used, you can now exchange the license by converting it to a BlueXP backup and recovery license, BlueXP classification license, or BlueXP tiering license.
...
=== BlueXP digital wallet update

We updated the *BlueXP digital wallet* by consolidating Cloud Volumes ONTAP licenses in a single tab.
...
* https://www.netapp.com/services/keystone/[Learn more about Keystone Subscriptions^].
...
A new Professional Package enables you to bundle Cloud Volumes ONTAP and BlueXP backup and recovery service by using an annual contract from the AWS Marketplace. Payment is per TiB. This subscription doesn't enable you to back up on-prem data.
