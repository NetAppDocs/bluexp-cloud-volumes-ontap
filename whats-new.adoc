...
=== Support ended for n1 series for new deployments in Google Cloud

The n1 series machines are no longer supported in new deployments of Cloud Volumes ONTAP in Google Cloud, and are only retained in old systems. New Cloud Volumes ONTAP deployments in Google Cloud are supported from the 9.8 release. We recommend that you switch to the n2 series machine types that are supported by Cloud Volumes ONTAP 9.8 and later.
...
=== Enhanced user experience for version selection
The enhanced upgrade flow of Cloud Volumes ONTAP through BlueXP notification now enables you to view and use the default, latest, and compatible version for upgrading your system. Additionally, you can nows select the latest patch or major version that is compatible with your current Cloud Volumes ONTAP instance, or manually enter the version for upgrade.
