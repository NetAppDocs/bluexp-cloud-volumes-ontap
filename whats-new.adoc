...
=== Placement group creation setting for AWS HA single availability zone
A new configuration setting is now available for placement group creation with AWS HA single availability zone deployments. You can now chose to bypass failed placement group creations and allow AWS HA single AZ deployments to complete successfully. 
...
A new configuration setting is now available so that you can create a link between a private DNS zone and a virtual network when using Azure Private Links. Creation is enabled by default.  
