...
=== Updates for Azure China regions 
Cloud Volumes ONTAP 9.11.1P4 is now supported in Azure China regions. Two new Azure China regions are supported:

* Azure China North 3
* Azure China East 3

You can upgrade existing Cloud Volumes ONTAP systems in China regions to 9.11.1P4. 

Note that only single node systems with BYOL are supported in China regions.

See all supported regions on the link:https://bluexp.netapp.com/cloud-volumes-global-regions[Cloud Volumes ONTAP Global Regions Maps page^].

=== Updates for AWS China regions
Cloud Volumes ONTAP 9.11.1 P4 is now supported in AWS China regions. You can upgrade existing Cloud Volumes ONTAP systems in China regions to 9.11.1P4.
...
Cloud Volumes ONTAP is now supported in cloud environments that have complete isolation from the internet. Only node-based licensing (BYOL) is supported in these environments. Capacity-based licensing is not supported.

To get started, manually install the Connector software, log in to the BlueXP console that's running on the Connector, and then deploy Cloud Volumes ONTAP.
