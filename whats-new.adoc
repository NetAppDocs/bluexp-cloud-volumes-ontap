...
The built-in data protection and security features of WORM (Write Once Read Many) and ARP (Autonomous Ransomware Protection) will be included with all Cloud Volumes ONTAP licenses at no additional cost. This will be applicable for new and existing BYOL and PAYGO/marketplace deployments for both node-based and capacity-based licensing models. The new pricing model applies to all cloud providers and deployment types, including single node and high availability (HA) configurations.

The following benefits will be available:

* Existing accounts with WORM and ARP will no longer be charged for these features. You will be billed only for Cloud Volumes ONTAP capacity usage as before. WORM and ARP will be excluded in future billing.
...
* For new accounts, WORM and ARP charges will be eliminated from any Cloud Volumes ONTAP offering.
