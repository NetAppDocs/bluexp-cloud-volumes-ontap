...
* For single availability zones, select a region, VNet, and subnet. ##In Cloud Volumes ONTAP 9.15.1 and later, zonal VM instances are installed in single availability zones. You need to select a region that supports this deployment. If the region selected by you does not support this deployment mode, then the previous non-zonal deployment model for locally redundant storage (LRS) is followed. For information about the regions that support this deployment mode, refer to...##
