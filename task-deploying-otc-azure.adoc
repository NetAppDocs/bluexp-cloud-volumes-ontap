...
+
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later leverage Virtual Machine Scale Sets in Flexible orchestration mode to deploy zonal VM instances in single availability zones. This also requires the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature to be activated for your subscription. For more information, refer to link:task-azure-high-availability-mode.html[Enable high availability mode in Azure].##
+
...
+
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later leverage Virtual Machine Scale Sets in Flexible orchestration mode to deploy zonal VM instances in single availability zones. For single node systems, you need to select a region and an availability zone that support Virtual Machine Scale Sets in Flexible orchestration mode. If the region or zone selected by you does not support this deployment mode, then the previous non-zonal deployment model is followed in locally redundant storage (LRS).##
...
. *Licensing*: Change the Cloud Volumes ONTAP version if required, and select a virtual machine type. ##Cloud Volumes ONTAP versions earlier than 9.15.1 do not support Virtual Machine Scale Sets to deploy VM instances in single availability zones. If you select an earlier ONTAP version, there might be errors in deployment. Similarly, if the selected VM type does not support Virtual Machine Scale Sets in Flexible orchestration mode, you might need to change the selection.##
+
NOTE: If a newer Release Candidate, General Availability, or patch release is available for the selected version, then BlueXP updates the system to that version when creating the working environment. For example, the update occurs if you select Cloud Volumes ONTAP 9.13.1 and 9.13.1 P4 is available. The update does not occur from one release to another—for example, from 9.12.1 to 9.13.1. 
...
* For single AZ, select a region, VNet, and subnet. ##Cloud Volumes ONTAP 9.15.1 and later leverage Virtual Machine Scale Sets in Flexible orchestration mode to deploy zonal VM instances in single availability zones. You need to select a region that supports Virtual Machine Scale Sets in Flexible orchestration mode. If the region selected by you does not support this deployment mode, then the previous non-zonal deployment model is followed in locally redundant storage (LRS). For information about the regions that support Virtual Machine Scale Sets, refer to...##
...
. *Usage Profile, Disk Type, and Tiering Policy*: Choose whether you want to enable storage efficiency features and change the volume tiering policy, if needed.
+
For more information, refer to link:https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile[Choose a volume usage profile^] and link:concept-data-tiering.html[Data tiering overview^].
