...
* ##For single availability zones, select an Azure region, an availability zone, VNet, and subnet. Cloud Volumes ONTAP 9.15.1 and later install VM instances in a single availability zone by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. You need to select a region that supports this deployment. If the region selected by you does not support zonal deployment, then the previous non-zonal deployment model for locally redundant storage (LRS) is followed. To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^].##
