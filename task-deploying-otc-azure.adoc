...
+
[NOTE]
##For single availability zones and locally redundant storage (LRS), you must select an availability zone. Cloud Volumes ONTAP 9.15.1 and later leverages Virtual Machine Scale Sets to deploy VMs in LRS and zone-redundant storage (ZRS) that utilizes the selected zone for VM deployment.##
