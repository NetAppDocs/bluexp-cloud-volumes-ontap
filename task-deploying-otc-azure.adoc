...
+
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later leverage Virtual Machine Scale Sets to deploy VMs in locally redundant storage (LRS) and zone-redundant storage (ZRS) that requires the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature to be activated for your subscription. For more information, refer to link:task-azure-high-availability-mode.html[Enable high availability mode in Azure].##
+
...
+
[NOTE]
##For single availability zones and locally redundant storage (LRS), you must select an availability zone.##
...
. *Licensing*: Change the Cloud Volumes ONTAP version if required, and select a virtual machine type. ##Cloud Volumes ONTAP versions earlier than 9.15.1 do not support Virtual Machine Scale Sets to deploy VMs in single availability zones. If you select an earlier ONTAP version, there might be errors in deployment. Similarly, if the selected VM type does not support Virtual Machine Scale Sets, you might need to change the selection.##
+
