...
[NOTE]
##For high-availability mode, Cloud Volumes ONTAP 9.15.1 and later support deployment of zonal VM instances in single availability zones. This also requires the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature to be activated for your subscription. For more information, refer to link:task-saz-feature.html[Enable HA deployment in single availability zones].##
+
* https://docs.netapp.com/us-en/bluexp-classification/concept-cloud-compliance.html[Learn more about BlueXP classification^]
* https://docs.netapp.com/us-en/bluexp-backup-recovery/concept-backup-to-cloud.html[Learn more about BlueXP backup and recovery^]
+
...
* For single availability zones, select a region, VNet, and subnet. ##For high-availability mode in Cloud Volumes ONTAP 9.15.1 and later, zonal VM instances are installed in single availability zones by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. You need to select a region that supports this deployment. If the region selected by you does not support this deployment mode, then the previous non-zonal deployment model for locally redundant storage (LRS) is followed.  To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^]. For information about the supported regions, refer to https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/[Products available by region].##
