...
+
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later support deployment of zonal VM instances in single availability zones. This also requires the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature to be activated for your subscription. For more information, refer to link:task-azure-high-availability-mode.html[Enable high availability mode in Azure].##
+
...
+
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later deploy zonal VM instances in single availability zones by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. For single node systems, you need to select a region and an availability zone that support this deployment. Else, the previous non-zonal deployment model is followed in locally redundant storage (LRS). To learn more, refer to the https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Virtual Machine Scale Sets documentation^].##
...
* For single availability zones, select a region, VNet, and subnet. ##In Cloud Volumes ONTAP 9.15.1 and later, zonal VM instances are installed in single availability zones by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. You need to select a region that supports this deployment. If the region selected by you does not support this deployment mode, then the previous non-zonal deployment model for locally redundant storage (LRS) is followed. For information about the regions that support this deployment mode, refer to...##
