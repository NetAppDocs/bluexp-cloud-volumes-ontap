...
[NOTE]
##Cloud Volumes ONTAP 9.15.1 and later deploy VM instances in locally-redundant storage (LRS) single availability zones. This also requires the `Microsoft.Compute/VMOrchestratorZonalMultiFD` feature to be activated for your subscription. For information about activating this feature, refer to link:task-saz-feature.html[Enable VMOrchestratorZonalMultiFD for single availability zones]. This feature facilitates the high-availability mode for all Cloud Volumes ONTAP HA pairs. For information about HA mode, refer to link:task-azure-high-availability-mode.html[Enable high availability mode in Azure].##
+
* https://docs.netapp.com/us-en/bluexp-classification/concept-cloud-compliance.html[Learn more about BlueXP classification^]
* https://docs.netapp.com/us-en/bluexp-backup-recovery/concept-backup-to-cloud.html[Learn more about BlueXP backup and recovery^]
+
...
* ##For single availability zones, select an Azure region, an availability zone, VNet, and subnet. Beginning with Cloud Volumes ONTAP 9.15.1, you can install VM instances in a single availability zones by using Azure Virtual Machine Scale Sets in Flexible orchestration mode. You need to select a region that supports this deployment. If the region selected by you does not support zonal deployment, then the previous non-zonal deployment model for LRS is followed. To learn more, refer to https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/[Microsoft Azure documentation: Virtual Machine Scale Sets documentation^].##
