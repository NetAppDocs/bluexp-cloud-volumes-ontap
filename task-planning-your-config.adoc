...
EBS disk size::
You need to choose an initial disk size when you launch a Cloud Volumes ONTAP system. After that, you can link:concept-storage-management.html[let Cloud Manager manage a system's capacity for you], but if you want to link:task-provisioning-storage.html#creating-aggregates[build aggregates yourself], be aware of the following:
...
TIP: The Connector also requires a system disk. https://docs.netapp.com/us-en/cloud-manager-connector/reference-connector-default-config.html[View details about the Connector's default configuration].
