...
* Enables capacity management features in both automatic and manual modes for the discovered systems.

.Before you begin
Ensure that the deployment is complete on the Azure marketplace. The BlueXP Connector can discover the systems only when the systems have been deployed and are available for discovery.
...
.Steps
In BlueXP, you follow the standard procedure to discover existing systems.

For information about discovering and adding existing systems to BlueXP, refer to link:task-adding-systems.html[Add an existing Cloud Volumes ONTAP system to BlueXP].

.After you finish
After the discovery is complete, you can view the systems in the BlueXP console. The systems are listed as working environments, and you can manage them from the BlueXP console. You can perform various management tasks, such as, expanding the aggregate, adding volumes, provisioning additional storage VMs and adding data LIFs to them, and changing the instance types.

.Related information

* link:task-verify-autosupport.html[Verify AutoSupport setup]
* link:task-tiering.html[Tier inactive data to low-cost object storage]
* link:concept-worm.html[Learn about WORM storage]
