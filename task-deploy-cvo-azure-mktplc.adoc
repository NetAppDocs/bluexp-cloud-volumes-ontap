...
* Replaces the free licenses of the discovered systems as regular capacity-based Freemium licenses. For information about Freemium licenses, refer to link:/concept-licensing.html#packages[Capacity-based licensing packages].
...
* Provisions systems and activates accounts for FabricPool tiering.
* Assigns the applicable ONTAP licenses on the Cloud Volumes ONTAP nodes. ##Reviewers, please provide more info.##
...
* Replaces the installed licenses with the new licenses for NFS, CIFS (SMB), iSCSI, ARP, SnapLock, and SnapMirror.##Reviewers, please provide more info.##
* Enables AutoSupport on the deployed systems.
* Activates write-once-read-many (WORM) storage.
* Removes the unused cloud resources added to the instances post deployment.
* Registers the instances to the NSS accounts used to discover them.

After BlueXP discovers and adds the systems, you can perform various management tasks, such as, expanding the aggregate, adding volumes, provisioning additional storage VMs and adding data LIFs to them, and changing the instance type.

[NOTE]
Azure marketplace direct deployment supports L8s_v3 instance types. However, BlueXP doesn't support this instance type for HA configurations, and won't be able to detect it during discovery.